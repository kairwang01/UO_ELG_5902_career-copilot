...
describe('Web3 platform config', () => {
  ...
  it('updates the platform-wide flag and writes an admin audit event', async () => {
    const cfg = await updateWeb3ConfigImpl('super-web3', { enabled: true });
    ...
    expect(audit.docs[0].data().details).toMatchObject({ enabled: true, network: 'sepolia', chain_id: 11155111 });
  });
