...
- **Marketing / Beta site:** `marketing/site-theme.css` + `marketing/design-tokens.ts` — the de-templated public pages (`marketing/pages/`, `marketing/components/`). This is the layer the redesign direction targets.

Design direction source: Confluence "Career CoPilot — Product & Design Direction (Beta redesign)"
(space `TCC`, page `9601025`) — reachable and reflected here. Companion files:
...
### Marketing / Beta tokens (`--site-*`)

Defined on `.beta-root` in `marketing/site-theme.css`; mirrored as TS constants in
`marketing/design-tokens.ts`. Use these for all public-site work.
...
| Layer | Family |
|-------|--------|
| Marketing / Beta | `system-ui, -apple-system, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif` (system stack — **no Google Fonts fetch** in the Beta shell) |
| App shell | `Inter` (`index.css` `body`) loaded via Google Fonts |
...
- Banned for display headlines: `Inter` 700/800 (`font-extrabold`) and any gradient text fill.
...
3. **Fake-score cards.** No "Resume Score 94%" floating card, no decorative score badges, no out-of-context metrics ("10x faster"). → Real product screenshot or a structured report mock that names the gaps, keywords, and role fit with explanation.
...
8. **`font-extrabold` Inter for display headlines / gradient text fills.** → Display weight 600, solid color.
...
11. **Hard-coded hex in marketing/Beta components.** → Use `--site-*` CSS vars or `marketing/design-tokens.ts`.
12. **University (uOttawa / Team / CaIoT) branding in hero/nav, or "AI-powered / Unlock / Empower / Transform / Seamless" filler copy.** → Footer academic credit only; concrete, outcome-specific copy.

### Lint targets (when an ESLint plugin is added)

Fail the build on, within marketing/Beta files:
`from-violet`, `from-indigo`, `to-purple`, `bg-indigo-600`,
`rounded-2xl`, `rounded-3xl`, `shadow-lg`, `shadow-xl`,
and `font-extrabold` + `Inter` in display contexts.
...
- Direction (Confluence): TCC / page 9601025 — "Product & Design Direction (Beta redesign)".
- `marketing/Design.md` — beta design-system narrative + page IA.
