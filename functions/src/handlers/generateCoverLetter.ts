...
export const generateCoverLetterFunction = onCall({ invoker: "public", timeoutSeconds: 180 }, async (request) => {
  ...
  if (data.resumeText.length > 100_000 || data.jobDescription.length > 100_000 || data.marketName.length > 120) {
    throw new HttpsError("invalid-argument", "Cover letter input is too long.");
  }
