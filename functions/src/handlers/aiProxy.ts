...
export const aiProxyFunction = onCall({ invoker: "public", timeoutSeconds: 180 }, async (request) => {
  ...
  try {
    ...
    const meteringPromise = spec.creditKey
      ? meterToolRun(uid, spec.creditKey, cost, { requestId }).then((deduction) => {
          ...
          return { creditCost: deduction.creditCost, charged: deduction.charged };
        })
      : recordFreeToolRun(uid, tool, { requestId }).then((run) => {
          if (run.duplicate) {
            throw new HttpsError("already-exists", "This AI request was already submitted. Please wait for the current result.");
          }
          return { creditCost: 0, charged: false };
        });
    ...
    if (meteringOutcome.status === "fulfilled") {
      ...
      charged = meteringOutcome.value.charged;
    } else {
