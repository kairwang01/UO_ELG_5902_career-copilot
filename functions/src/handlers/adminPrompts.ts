...
/**
 * Writes the prompt content into platform_config/prompts (the runtime read path)
 * and flushes the in-memory cache.  Mirrors exactly what adminUpdatePrompt did.
 */
async function writeToRuntime(promptKey: string, content: string): Promise<void> {
  const ref = db.collection(PLATFORM_CONFIG_COLLECTION).doc(PLATFORM_DOCS.prompts);
  await ref.set({ [promptKey]: content }, { merge: true });
...
export const adminPublishPromptFunction = onCall({ invoker: "public" }, async (request) => {
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (data.status === "published") {
      throw new HttpsError("failed-precondition", "This version is already published.");
    }
  ...
  // Write to the runtime path (outside transaction — idempotent set)
  await writeToRuntime(promptKey, content);
...
export const adminUpdatePromptFunction = onCall({ invoker: "public" }, async (request) => {
  ...
  // Write to runtime read path (same as the original implementation)
  await writeToRuntime(key, template);
...
export const adminResetPromptFunction = onCall({ invoker: "public" }, async (request) => {
  ...
  // Archive any currently-published version in the versions collection
  await db.runTransaction(async (tx) => {
    await archivePreviousPublished(tx, key);
  });
