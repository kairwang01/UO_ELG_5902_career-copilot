...
HTTP_CODE=$(curl -s -o /tmp/smoke-test-response.json -w "%{http_code}" \
...
FUNCTION_RESPONSE=$(cat /tmp/smoke-test-response.json 2>/dev/null || echo "")
...
SCORE=$(echo "$FUNCTION_RESPONSE" | grep -o '"score":[0-9]*' | head -1 | cut -d: -f2)
if [ -n "$SCORE" ]; then
  echo ""
  echo "  ✓ Phase A PASSED"
