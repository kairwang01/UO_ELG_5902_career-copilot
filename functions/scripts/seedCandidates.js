 ...
 *
 * Idempotent per email: re-running updates the existing test users.
 */

/* eslint-disable no-console */
const admin = require("firebase-admin");

admin.initializeApp({ projectId: "career-copilot-a3168" });

const COUNT = Math.min(parseInt(process.argv[2] || "6", 10) || 6, 20);
const PASSWORD = "CandidateTest!2026";
...
async function upsertCandidate(i) {
  ...
  try {
    ...
    user = await auth.createUser({ email, password: PASSWORD, displayName: p.name });
...
(async () => {
  console.log(`Seeding ${COUNT} test candidates (password: ${PASSWORD}) …`);
  const out = [];
  for (let i = 0; i < COUNT; i++) {
    out.push(await upsertCandidate(i));
  }
  const staked = out.filter((o) => o.staked).length;
  console.log(`\nDone. ${out.length} candidates (${staked} verified/staked for the Verified Talent rail).`);
  console.log("Discover Talent will now return matches for engineering-style job descriptions.");
  process.exit(0);
})().catch((err) => {
  console.error(err);
  process.exit(1);
