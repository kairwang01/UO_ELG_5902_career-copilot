 ...
 *   2. Make sure you have a business/employer account already created in Firebase
 *      Auth + Firestore (role: 'employer').  Use the UID of that account as the
 *      first argument.
 *
 * USAGE (run from the functions/ directory)
 *   node scripts/seedJobs.js <employerUid> [count=12]
 *
 *   Examples:
 ...
 *   - Titles cycle deterministically over the TEMPLATES array and are shuffled
 *     when count > TEMPLATES.length so you get varied results even at high counts.
 */
...
const employerUid = process.argv[2];
const count = parseInt(process.argv[3] || "12", 10);

if (!employerUid) {
  console.error("Usage: node scripts/seedJobs.js <employerUid> [count=12]");
  process.exit(1);
}
if (isNaN(count) || count < 1 || count > 200) {
  console.error("count must be a number between 1 and 200");
  process.exit(1);
}
...
/** Build a list of exactly `n` templates, cycling + shuffling to fill. */
...
(async () => {
  ...
  // Firestore batch limit is 500 writes; split into chunks if needed
  ...
  for (const chunk of chunks) {
    ...
    for (const job of chunk) {
      ...
      // Only fields allowed by validJobPosting hasOnly rule:
      //   required: employer_id, title, is_active, created_at, updated_at
      //   optional: description, location, salary_range
      const doc = {
  ...
  console.log(`\nSeeded ${createdIds.length} job posting(s) for employer ${employerUid}:\n`);
  createdIds.forEach((id) => console.log(`  job_postings/${id}`));
  console.log(
    "\nTo clean up, delete these document IDs from the Firestore console" +
    "\nor via: firebase firestore:delete job_postings/<id> --project " + projectId
  );
  process.exit(0);
})().catch((e) => {
