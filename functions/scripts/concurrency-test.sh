...
UID=$(echo "$AUTH_RESPONSE" | grep -o '"localId":"[^"]*"' | cut -d'"' -f4)

if [ -z "$ID_TOKEN" ] || [ -z "$UID" ]; then
...
FIRESTORE_DOC_URL="${FIRESTORE_URL}/v1/projects/${PROJECT_ID}/databases/(default)/documents/users/${UID}"
...
echo "  ✓ Seeded users/${UID}.credits = ${START_CREDITS}"
...
FINAL_CREDITS=$(echo "$FINAL_DOC" | grep -o '"credits":{[^}]*}' | grep -o '"integerValue":"[^"]*"' | cut -d'"' -f4)

if [ -z "$FINAL_CREDITS" ]; then
  # Try doubleValue (Firestore may return as double)
  FINAL_CREDITS=$(echo "$FINAL_DOC" | grep -o '"credits":{[^}]*}' | grep -o '"doubleValue":[0-9.]*' | cut -d: -f2 | cut -d. -f1)
fi
...
if [ "$FINAL_CREDITS" = "$EXPECTED_FINAL" ]; then
  echo ""
  echo "  ✓ M9 PASSED"
