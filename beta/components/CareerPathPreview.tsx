...
const statusLabel = (t: (k: string) => string, status: TimelineStatus) => {
  if (status === 'done') return t('beta_timeline_done');
  if (status === 'in_progress') return t('beta_timeline_in_progress');
  return t('beta_timeline_pending');
};

const statusDot = (status: TimelineStatus) => {
  if (status === 'done') return 'bg-[var(--beta-ready)]';
  if (status === 'in_progress') return 'bg-[var(--beta-action)] ring-4 ring-[var(--beta-action)]/20';
  return 'bg-[var(--beta-border)]';
};
...
export const CareerPathPreview: React.FC<CareerPathPreviewProps> = ({ t, compact }) => {
  ...
  return (
    ...
    >
      <div className="mb-6">
        <p className="text-xs font-medium text-[var(--beta-text-muted)] mb-3">{t('beta_career_timeline')}</p>
            ...
            <div key={step.id} className="flex gap-3">
              <div className="flex flex-col items-center">
                ...
                {i < plan.timeline.length - 1 && (
                  <div className="w-px flex-1 min-h-[2rem] bg-[var(--beta-border)] my-1" />
                )}
              </div>
              <div className="pb-4 min-w-0 flex-1">
                <div className="flex flex-wrap items-center gap-2">
                  <p className="text-sm font-medium">{step.label}</p>
                  <span className="text-[10px] px-1.5 py-0.5 rounded bg-[var(--beta-surface-muted)] text-[var(--beta-text-muted)]">
                    {statusLabel(t, step.status)}
                  </span>
                </div>
                {step.detail && (
                  <p className="text-xs text-[var(--beta-text-muted)] mt-0.5">{step.detail}</p>
                )}
              </div>
            </div>
          ))}
        ...
        <div className="border-t border-[var(--beta-border)] pt-4">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {plan.fourWeekPlan.map((w) => (
              <div
                key={w.week}
                className={`border rounded-[var(--beta-radius)] p-3 ${
                  w.status === 'in_progress'
                    ? 'border-[var(--beta-action)] bg-[var(--beta-surface-muted)]'
                    : 'border-[var(--beta-border)]'
                }`}
              >
                <div className="flex items-center justify-between gap-2 mb-1">
                  <div className="flex items-center gap-2">
                    <span className="w-6 h-6 rounded-full bg-[var(--beta-action)] text-white text-xs flex items-center justify-center font-medium shrink-0">
                      {w.week}
                    </span>
                    <span className="text-sm font-medium">{w.focus}</span>
                  </div>
                  <span className="text-[10px] text-[var(--beta-text-muted)] shrink-0">
