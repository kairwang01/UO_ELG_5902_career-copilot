import React from 'react';
import { caseSnapshots } from '../mock/caseSnapshots';

interface CaseSnapshotsProps {
  t: (key: string) => string;
}

export const CaseSnapshots: React.FC<CaseSnapshotsProps> = ({ t }) => (
  <section id="cases-section" className="py-[var(--beta-section)] bg-[var(--beta-surface-muted)]">
    <div className="max-w-6xl mx-auto px-4 sm:px-6">
      <h2 className="text-2xl font-semibold mb-2">{t('beta_cases_title')}</h2>
      <p className="text-[var(--beta-text-muted)] mb-8 max-w-2xl">{t('beta_cases_subtitle')}</p>
      <div className="grid md:grid-cols-3 gap-4">
        {caseSnapshots.map((c) => (
          <article
            key={c.id}
            className="border border-[var(--beta-border)] rounded-[var(--beta-radius)] p-5 bg-[var(--beta-surface)]"
          >
            <span className="text-xs font-medium text-[var(--beta-action)]">{t(c.tagKey)}</span>
            <h3 className="font-semibold mt-2 mb-2">{t(c.titleKey)}</h3>
            <p className="text-sm text-[var(--beta-text-muted)] mb-4">{t(c.outcomeKey)}</p>
            <p className="text-sm font-medium border-t border-[var(--beta-border)] pt-3">
              {t(c.metricKey)}
            </p>
          </article>
        ))}
      </div>
    </div>
  </section>
);
