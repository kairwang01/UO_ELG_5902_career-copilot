...
function startVite() {
  const child = spawn(
    ...
    ['vite', '--host', '127.0.0.1', '--port', '4186', '--strictPort'],
    {
      cwd: ROOT,
      env: viteEnv,
...
async function assertWeb3PreviewFlow(browser, db, uid) {
  ...
  try {
    ...
    await page.getByRole('heading', { name: /^settings$/i }).waitFor({ timeout: 20_000 });
    await page.getByText('Verified Identity').waitFor({ state: 'detached', timeout: 20_000 });
    console.log('  ✓ disabled config hides candidate Web3 surface');

    await setWeb3Config(db, { enabled: true, preview_mode: true });
    await page.reload({ waitUntil: 'domcontentloaded' });
    await page.getByText('Verified Identity').waitFor({ timeout: 20_000 });
    await page.locator('[role="note"]').filter({ hasText: /Sepolia preview/i }).waitFor({ timeout: 20_000 });
