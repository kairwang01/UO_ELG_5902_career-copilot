...
const authEmulator = new URL(AUTH_EMULATOR_URL);
const storageEmulatorHost = process.env.FIREBASE_STORAGE_EMULATOR_HOST || `127.0.0.1:${process.env.VITE_FIREBASE_STORAGE_EMULATOR_PORT || '9197'}`;
const [storageEmulatorHostname, storageEmulatorPort = '9197'] = storageEmulatorHost.split(':');
...
process.env.FIRESTORE_EMULATOR_HOST = process.env.FIRESTORE_EMULATOR_HOST || `127.0.0.1:${process.env.VITE_FIRESTORE_EMULATOR_PORT || '8080'}`;
process.env.FIREBASE_AUTH_EMULATOR_HOST = process.env.FIREBASE_AUTH_EMULATOR_HOST || `${authEmulator.hostname}:${authEmulator.port}`;
...
const viteEnv = {
  ...
  VITE_FIREBASE_STORAGE_EMULATOR_HOST: process.env.VITE_FIREBASE_STORAGE_EMULATOR_HOST || storageEmulatorHostname,
  VITE_FIREBASE_STORAGE_EMULATOR_PORT: process.env.VITE_FIREBASE_STORAGE_EMULATOR_PORT || storageEmulatorPort,
};
...
function startVite() {
  ...
  const child = spawn(
    ...
    ['vite', '--host', url.hostname, '--port', url.port || '4183', '--strictPort'],
    {
      cwd: ROOT,
      env: viteEnv,
...
async function assertSavedToolResultRestore(page, toolKey, label, resultSelector) {
  ...
  const saveStatus = await resultRoot.locator('[data-qa="tool-save-status"]').innerText({ timeout: 10_000 });
  assert(saveStatus.includes('Saved'), `${label}: expected saved status after generation`);

  await returnToLibrary(page);
  await openTool(page, toolKey);
  await page.locator(resultSelector).waitFor({ timeout: 20_000 });
  const restoredStatus = await page.locator(resultSelector).locator('[data-qa="tool-save-status"]').innerText({ timeout: 10_000 });
  assert(restoredStatus.includes('Saved'), `${label}: expected saved result to restore on reopen`);
  await page.locator(resultSelector).locator('[data-qa="tool-remove-saved"]').click();
  await waitForToolResultClear(toolKey, label);
  const clearedStatus = await page.locator(resultSelector).locator('[data-qa="tool-save-status"]').innerText({ timeout: 10_000 });
  assert(clearedStatus.includes('Not saved'), `${label}: expected cleared save status`);
}
...
async function runResumeFormatter(page) {
  ...
  await assertSavedToolResultRestore(page, 'resume-formatter', 'resume formatter', '[data-qa="resume-formatter"][data-qa-resume-formatter-state="result"]');
...
async function runOpportunityFinder(page) {
  ...
  const handoffResult = await waitForToolResultWrite('cover-letter', 'opportunity cover-letter handoff');
  await page.locator('[data-qa="cover-letter-tool"][data-qa-tool-state="result"]').waitFor({ timeout: 60_000 });
  const handoffContext = String(handoffResult.result?.jobDescription || '');
...
async function runEnglishProWritten(page) {
  ...
  const saveStatus = await page.locator('[data-qa="english-pro-written-result"] [data-qa="tool-save-status"]').innerText();
  assert(saveStatus.includes('Saved'), 'english pro: expected written result save status');
  await returnToLibrary(page);

  await openTool(page, 'english-pro');
  await page.locator('[data-qa="english-pro-tool"][data-qa-english-mode="written"]').waitFor({ timeout: 20_000 });
  await page.locator('[data-qa="english-pro-written-result"]').waitFor({ timeout: 20_000 });
  const restoredStatus = await page.locator('[data-qa="english-pro-written-result"] [data-qa="tool-save-status"]').innerText();
  assert(restoredStatus.includes('Saved'), 'english pro: expected saved written result to restore on reopen');
  ...
  const clearedStatus = await page.locator('[data-qa="english-pro-written-result"] [data-qa="tool-save-status"]').innerText();
  assert(clearedStatus.includes('Not saved'), 'english pro: expected cleared save status');
...
async function runEnglishProListening(page) {
  ...
  const saveStatus = await page.locator('[data-qa="english-pro-listening-result"] [data-qa="tool-save-status"]').innerText();
  assert(saveStatus.includes('Saved'), 'english pro: expected listening result save status');
  await returnToLibrary(page);

  await openTool(page, 'english-pro');
  await page.locator('[data-qa="english-pro-tool"][data-qa-english-mode="listening"]').waitFor({ timeout: 20_000 });
  await page.locator('[data-qa="english-pro-listening-result"]').waitFor({ timeout: 20_000 });
  const restoredStatus = await page.locator('[data-qa="english-pro-listening-result"] [data-qa="tool-save-status"]').innerText();
  assert(restoredStatus.includes('Saved'), 'english pro: expected saved listening result to restore on reopen');
  ...
  const clearedStatus = await page.locator('[data-qa="english-pro-listening-result"] [data-qa="tool-save-status"]').innerText();
  assert(clearedStatus.includes('Not saved'), 'english pro: expected cleared listening save status');
...
async function runEnglishProReadingAnalysis(page) {
  ...
  const saveStatus = await page.locator('[data-qa="english-pro-reading-result"] [data-qa="tool-save-status"]').innerText();
  assert(saveStatus.includes('Saved'), 'english pro: expected reading result save status');
  ...
  const restoredStatus = await page.locator('[data-qa="english-pro-reading-result"] [data-qa="tool-save-status"]').innerText();
  assert(restoredStatus.includes('Saved'), 'english pro: expected saved reading result to restore on reopen');
  ...
  const clearedStatus = await page.locator('[data-qa="english-pro-reading-result"] [data-qa="tool-save-status"]').innerText();
  assert(clearedStatus.includes('Not saved'), 'english pro: expected cleared reading save status');
