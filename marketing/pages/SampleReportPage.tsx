...
const generationSteps = [
  ...
  'Matching Product Manager requirements',
...
const ReportGenerationState: React.FC<{ activeStep: number; onSkip: () => void }> = ({ activeStep, onSkip }) => {
  ...
  return (
    <div className="rounded-[calc(var(--site-radius)*2)] border border-[var(--site-border)] bg-white p-5 sm:p-6 shadow-sm">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-2xl text-sm leading-7 text-[var(--site-text-muted)]">
            This mirrors the logged-in analysis flow: we read the resume, check structure, compare it to the target role, then assemble the report.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="mt-6 h-2 overflow-hidden rounded-full bg-[var(--site-surface-muted)]">
        <div
...
export const SampleReportPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const timers = generationSteps.map((_, index) =>
      window.setTimeout(() => {
        setActiveStep(index);
      }, index * 750),
    );
    const readyTimer = window.setTimeout(() => {
      setReportReady(true);
    }, generationSteps.length * 750 + 350);
      ...
      <section className="bg-[linear-gradient(180deg,#f8fafc_0%,#ffffff_52%,#f8fafc_100%)] py-10 sm:py-16 lg:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-8 sm:mb-12">
          <div className="grid gap-8 lg:grid-cols-[0.85fr_1.15fr] lg:items-end">
            <div>
              ...
              <h1 className="mt-5 max-w-3xl text-[clamp(2rem,5vw,4rem)] font-bold leading-[1.02] tracking-[-0.055em] text-[var(--site-text)]">
                {t('site_sample_title')}
              </h1>
            ...
            <div className="rounded-[calc(var(--site-radius)*2)] border border-[var(--site-border)] bg-white p-5 shadow-sm">
              ...
              <div className="grid gap-4 sm:grid-cols-3">
                ...
                <div>
                  <p className="text-xs text-[var(--site-text-muted)]">Target role</p>
                  <p className="mt-1 font-semibold text-[var(--site-text)]">{sampleReport.targetRole}</p>
                </div>
                <div>
                  <p className="text-xs text-[var(--site-text-muted)]">Status</p>
                  <p className={`mt-1 font-semibold ${reportReady ? 'text-[var(--site-ready)]' : 'text-[var(--site-action)]'}`}>
                    {reportReady ? 'Analysis complete' : generationSteps[activeStep]}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            <div className="rounded-[calc(var(--site-radius)*2)] border border-[var(--site-border)] bg-white p-5 sm:p-6 shadow-sm">
              ...
              <div className="grid gap-5 sm:grid-cols-[12rem_1fr] sm:items-center">
                <div className={`rounded-[calc(var(--site-radius)*2)] border p-5 text-center ${scoreTone(sampleReport.atsReadiness)}`}>
                  <p className="text-xs font-semibold uppercase tracking-[0.16em]">Overall score</p>
                ...
                <div>
                  <h2 className="text-2xl font-bold tracking-[-0.035em] text-[var(--site-text)]">
                    Product Manager fit is close, but positioning needs work.
                  </h2>
          ...
          <div className="rounded-[calc(var(--site-radius)*2)] border border-[var(--site-border)] bg-[var(--site-text)] p-6 text-center text-white">
            <h2 className="text-2xl font-bold tracking-[-0.035em]">Run this on your own resume</h2>
            <p className="mt-2 text-sm text-white/70">Upload a resume and get the same structured score, fixes, keywords, and next-step tools inside your workspace.</p>
