...
const InterviewPrep: React.FC<InterviewPrepProps> = ({ resumeText, market, initialInput = '', openTool, t }) => {
  ...
  const runTool = async () => {
    ...
    setResult(null);
    try {
      const apiResult = await generateCandidatePrepKit(resumeText, jobDescription.trim(), {
        targetRole: role,
        marketName: market,
  ...
  const formatForDownload = (res: CandidatePrepKit): string => {
    ...
    let content = `# Interview Prep Brief${role ? `: ${role}` : ''}\n\n`;
    if (res.sourceCoverage) content += `> ${res.sourceCoverage}\n\n`;
    ...
    if (ranked.length) {
      ranked.forEach((q, i) => {
        ...
        content += `   - Why: ${q.rationale}\n`;
        content += `   - Signal: frequency ${normFrequency(q.frequency)}, ${normRecency(q.recency)}, evidence ${normEvidence(q.evidenceLevel)}\n`;
      });
    } else {
    ...
    if (anchors.length || (res.followUpChains ?? []).length) {
      ...
      (res.followUpChains ?? []).forEach((chain) => {
        ...
        if (chain.watchFor) content += `_Watch for: ${chain.watchFor}_\n`;
      });
    }
    ...
    if (gaps.length || (res.weakSpots ?? []).length) {
      ...
      if (gaps.length) {
        gaps.forEach((g) => {
          content += `- (${normSeverity(g.severity)}) ${g.area}: ${g.risk}\n  Mitigation: ${g.mitigation}\n`;
        });
      } else {
              ...
              <div>
                <div className="mb-2 flex items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    className="text-sm font-semibold text-teal-700 transition hover:text-teal-800 dark:text-teal-300 dark:hover:text-teal-200"
                  >
                    {t('try_example')}
                  </button>
                </div>
  ...
  const renderResult = () => {
    ...
    return (
      <div data-qa="interview-prep-tool" data-qa-tool-state="result" className="mx-auto max-w-7xl space-y-5 animate-fade-in">
        <SavedResultBar
          t={t}
          canSave={canSave}
          isSaved={fromSaved}
                    ...
                    <article key={index} className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-950">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        ...
                        <button
                          ...
                          className="inline-flex min-h-9 items-center justify-center gap-1.5 rounded-lg border border-violet-200 bg-violet-50 px-3 py-1.5 text-xs font-semibold text-violet-700 transition hover:bg-violet-100 dark:border-violet-900/60 dark:bg-violet-950/30 dark:text-violet-200 dark:hover:bg-violet-900/40"
                        >
