...
const InputMethodButton: React.FC<{ icon: React.ReactNode; label: string; active: boolean; onClick: () => void; }> = ({ icon, label, active, onClick }) => (
    <button
      type="button"
      onClick={onClick}
      className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 text-sm font-semibold rounded-lg transition-all duration-200 border-2 ${active ? 'bg-blue-700 text-white border-blue-700 shadow-md' : 'bg-white dark:bg-slate-700 text-gray-700 dark:text-gray-200 border-gray-200 dark:border-slate-600 hover:bg-gray-100 dark:hover:bg-slate-600 hover:border-blue-300 dark:hover:border-blue-500'}`}
    >
      {icon}
      {label}
    </button>
...
const UploadSection: React.FC<UploadSectionProps> = ({ resumeText, setResumeText, resumeImages, setResumeImages, onInitiateAnalysis, isLoading, error, setError, market, setMarket, t }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 border border-gray-200/80 dark:border-slate-700/80 rounded-2xl p-6 sm:p-8 lg:p-12 shadow-lg animate-slide-in-up">
      <div className="text-center mb-8">
        <h2 className="text-3xl font-extrabold text-gray-900 dark:text-gray-100 tracking-tight">{t('upload_title')}</h2>
        <p className="mt-2 text-lg text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">{t('upload_subtitle')}</p>
      </div>
      
      <form onSubmit={handleFormSubmit} className="space-y-6 max-w-3xl mx-auto">
        
        <div className="space-y-2">
            <label htmlFor="market-select" className="flex items-center justify-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300">
            ...
            <select
                ...
                className="w-full max-w-xs mx-auto block bg-white dark:bg-slate-700 border-2 border-gray-200 dark:border-slate-600 text-gray-800 dark:text-gray-200 text-base rounded-xl focus:ring-blue-500 focus:border-blue-500 p-3 transition shadow-sm"
            >
        ...
        <div className="flex justify-center mb-6 gap-2 sm:gap-4 p-2 bg-gray-100 dark:bg-slate-900/50 rounded-xl">
        ...
        {error && <div className="bg-red-100 text-red-800 p-4 rounded-r-lg"><p>{error}</p></div>}
        {infoMessage && <div className="bg-blue-100 text-blue-800 p-4 rounded-r-lg"><p>{infoMessage}</p></div>}
        
        <div className="text-center pt-4">
            {!isAIMode && <p className="text-red-500 font-semibold mb-2 text-sm">Enable AI Mode from the top menu to use Analysis features.</p>}
            <button 
              type="submit" 
              disabled={analysisButtonDisabled || !isAIMode} 
              className="w-full max-w-xs flex items-center justify-center bg-blue-700 hover:bg-blue-800 disabled:bg-blue-400 text-white font-bold py-4 px-6 rounded-xl shadow-lg hover:shadow-xl transition-all mx-auto"
            >
