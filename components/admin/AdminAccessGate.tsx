...
/**
 * Resilience helper for AdminPortal: if adminWhoAmI fails with permission-denied
 * during the rollout window (e.g. function not yet deployed to this region) but
 * the legacy adminCheckAccess passes, fall back to treating the user as 'admin'.
 * The server enforces the real matrix — this is UI-only degraded mode.
 */
export const resolveRoleWithFallback = async (
  whoAmI: () => Promise<{ role: 'super' | 'admin' | 'reviewer' }>,
): Promise<'super' | 'admin' | 'reviewer'> => {
  try {
    const r = await whoAmI();
    return r.role;
  } catch (err: unknown) {
    // permission-denied or function-not-found during rollout → default to 'admin'
