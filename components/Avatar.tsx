...
const Avatar: React.FC<AvatarProps> = ({ url, size, onUpload }) => {
  ...
  useEffect(() => {
    if (url) {
      // Use getPublicUrl to get a direct link to the image in Supabase storage.
      const { data } = supabase.storage.from('avatars').getPublicUrl(url);
      // Append a timestamp as a query parameter to act as a cache-buster.
      // This forces the browser to re-fetch the image after an upload, ensuring the new avatar is shown.
      ...
      >
        ...
        {onUpload && (
            <span className="absolute bottom-0 right-0">
                <label htmlFor="single" className="cursor-pointer bg-blue-600 hover:bg-blue-700 text-white rounded-full p-2 shadow-md inline-block">
