...
test('super admin can re-enter a provider key to recover AI', async ({ page }) => {
  ...
  // 2) Go to Models & Keys and save a Gemini key with a BLANK fallback model —
  //    the exact shape that used to 500. It must succeed (no error surfaced).
  await page.getByText('Models & Keys', { exact: true }).first().click();
  await expect(page.locator('#provider-select')).toBeVisible({ timeout: 20_000 });
  await page.locator('#provider-key').fill('AIzaRecoveryKeyForE2E123');
  ...
  await expect(page.locator('#provider-key')).toHaveValue('', { timeout: 15_000 });
  await expect(page.getByText(/AIza.{2,10}123/).first()).toBeVisible({ timeout: 15_000 });
});
